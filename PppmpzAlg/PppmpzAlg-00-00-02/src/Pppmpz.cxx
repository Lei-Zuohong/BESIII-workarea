...
Pppmpz::Pppmpz(const std::string &name, ISvcLocator *pSvcLocator) : Algorithm(name, pSvcLocator)
{
	...
	declareProperty("do_4c_e", job_do_4c_e = 1);
	declareProperty("do_4c_k", job_do_4c_k = 1);
	declareProperty("do_4c_m", job_do_4c_m = 1);
	declareProperty("do_selection", job_selection = 1);
}
#pragma endregion
	...
	if (job_do_4c)
	{
		...
		else
		{
			...
			if (m_tuple_fit4c)
			{
				...
				status = m_tuple_fit4c->addItem("chisq_e", fit4c_chisq_e);
				status = m_tuple_fit4c->addItem("chisq_k", fit4c_chisq_k);
				status = m_tuple_fit4c->addItem("chisq_m", fit4c_chisq_m);
				// For 00 track
...
#pragma endregion
#pragma region init_selection
	if (job_selection)
	{
		NTuplePtr nt6(ntupleSvc(), "FILE1/selection");
		if (nt6)
		{
			m_tuple_selection = nt6;
		}
		else
		{
			m_tuple_selection = ntupleSvc()->book("FILE1/selection", CLID_ColumnWiseTuple, "ks N-Tuple example");
			if (m_tuple_selection)
			{
				// Topology信息
				status = m_tuple_selection->addItem("runID", runNo);
				status = m_tuple_selection->addItem("eventID", event);
				status = m_tuple_selection->addItem("indexmc", m_idxmc, 0, 100);
				status = m_tuple_selection->addIndexedItem("pdgid", m_idxmc, m_pdgid);
				status = m_tuple_selection->addIndexedItem("motheridx", m_idxmc, m_motheridx);
				status = m_tuple_selection->addItem("flag1", flag1);
				status = m_tuple_selection->addItem("flag2", flag2);
				status = m_tuple_selection->addItem("flag3", flag3);
				// For Cut
				status = m_tuple_selection->addItem("chisq", fit4c_chisq);
				status = m_tuple_selection->addItem("chisq_0g", fit4c_chisq_0g);
				status = m_tuple_selection->addItem("chisq_1g", fit4c_chisq_1g);
				status = m_tuple_selection->addItem("chisq_3g", fit4c_chisq_3g);
				status = m_tuple_selection->addItem("chisq_4g", fit4c_chisq_4g);
				status = m_tuple_selection->addItem("chisq_e", fit4c_chisq_e);
				status = m_tuple_selection->addItem("chisq_k", fit4c_chisq_k);
				status = m_tuple_selection->addItem("chisq_m", fit4c_chisq_m);
				// For 00 track
				status = m_tuple_selection->addItem("gamma1_heli", fit4c_gamma1_heli);
				status = m_tuple_selection->addItem("gamma2_heli", fit4c_gamma2_heli);
				status = m_tuple_selection->addItem("a_pippim", fit4c_a_pippim);
				// For +- track
				status = m_tuple_selection->addItem("pip_ep", fit4c_pip_ep);
				status = m_tuple_selection->addItem("pim_ep", fit4c_pim_ep);
				// For track
				status = m_tuple_selection->addItem("pip_m", fit4c_pip_m);
				status = m_tuple_selection->addItem("pip_p", fit4c_pip_p);
				status = m_tuple_selection->addItem("pip_a", fit4c_pip_a);
				status = m_tuple_selection->addItem("pip_pe", fit4c_pip_pe);
				status = m_tuple_selection->addItem("pip_px", fit4c_pip_px);
				status = m_tuple_selection->addItem("pip_py", fit4c_pip_py);
				status = m_tuple_selection->addItem("pip_pz", fit4c_pip_pz);
				status = m_tuple_selection->addItem("pim_m", fit4c_pim_m);
				status = m_tuple_selection->addItem("pim_p", fit4c_pim_p);
				status = m_tuple_selection->addItem("pim_a", fit4c_pim_a);
				status = m_tuple_selection->addItem("pim_pe", fit4c_pim_pe);
				status = m_tuple_selection->addItem("pim_px", fit4c_pim_px);
				status = m_tuple_selection->addItem("pim_py", fit4c_pim_py);
				status = m_tuple_selection->addItem("pim_pz", fit4c_pim_pz);
				status = m_tuple_selection->addItem("gamma1_m", fit4c_gamma1_m);
				status = m_tuple_selection->addItem("gamma1_p", fit4c_gamma1_p);
				status = m_tuple_selection->addItem("gamma1_a", fit4c_gamma1_a);
				status = m_tuple_selection->addItem("gamma1_pe", fit4c_gamma1_pe);
				status = m_tuple_selection->addItem("gamma1_px", fit4c_gamma1_px);
				status = m_tuple_selection->addItem("gamma1_py", fit4c_gamma1_py);
				status = m_tuple_selection->addItem("gamma1_pz", fit4c_gamma1_pz);
				status = m_tuple_selection->addItem("gamma2_m", fit4c_gamma2_m);
				status = m_tuple_selection->addItem("gamma2_p", fit4c_gamma2_p);
				status = m_tuple_selection->addItem("gamma2_a", fit4c_gamma2_a);
				status = m_tuple_selection->addItem("gamma2_pe", fit4c_gamma2_pe);
				status = m_tuple_selection->addItem("gamma2_px", fit4c_gamma2_px);
				status = m_tuple_selection->addItem("gamma2_py", fit4c_gamma2_py);
				status = m_tuple_selection->addItem("gamma2_pz", fit4c_gamma2_pz);
				status = m_tuple_selection->addItem("piz_m", fit4c_piz_m);
				status = m_tuple_selection->addItem("piz_p", fit4c_piz_p);
				status = m_tuple_selection->addItem("piz_a", fit4c_piz_a);
				status = m_tuple_selection->addItem("piz_pe", fit4c_piz_pe);
				status = m_tuple_selection->addItem("piz_px", fit4c_piz_px);
				status = m_tuple_selection->addItem("piz_py", fit4c_piz_py);
				status = m_tuple_selection->addItem("piz_pz", fit4c_piz_pz);
				status = m_tuple_selection->addItem("pipm_m", fit4c_pipm_m);
				status = m_tuple_selection->addItem("pipm_p", fit4c_pipm_p);
				status = m_tuple_selection->addItem("pipm_a", fit4c_pipm_a);
				status = m_tuple_selection->addItem("pipm_pe", fit4c_pipm_pe);
				status = m_tuple_selection->addItem("pipm_px", fit4c_pipm_px);
				status = m_tuple_selection->addItem("pipm_py", fit4c_pipm_py);
				status = m_tuple_selection->addItem("pipm_pz", fit4c_pipm_pz);
				status = m_tuple_selection->addItem("pipz_m", fit4c_pipz_m);
				status = m_tuple_selection->addItem("pipz_p", fit4c_pipz_p);
				status = m_tuple_selection->addItem("pipz_a", fit4c_pipz_a);
				status = m_tuple_selection->addItem("pipz_pe", fit4c_pipz_pe);
				status = m_tuple_selection->addItem("pipz_px", fit4c_pipz_px);
				status = m_tuple_selection->addItem("pipz_py", fit4c_pipz_py);
				status = m_tuple_selection->addItem("pipz_pz", fit4c_pipz_pz);
				status = m_tuple_selection->addItem("pimz_m", fit4c_pimz_m);
				status = m_tuple_selection->addItem("pimz_p", fit4c_pimz_p);
				status = m_tuple_selection->addItem("pimz_a", fit4c_pimz_a);
				status = m_tuple_selection->addItem("pimz_pe", fit4c_pimz_pe);
				status = m_tuple_selection->addItem("pimz_px", fit4c_pimz_px);
				status = m_tuple_selection->addItem("pimz_py", fit4c_pimz_py);
				status = m_tuple_selection->addItem("pimz_pz", fit4c_pimz_pz);
			}
			else
			{
				log << MSG::ERROR << "Cannot book N-tuple:" << long(m_tuple_selection) << endmsg;
				return StatusCode::FAILURE;
			}
		}
	}
...
#pragma region section_vertex fit
	...
	wpim = WTrackParameter(mpipm, pimTrk->getZHelix(), pimTrk->getZError());	  // wvkpTrk = WTrackParameter(mk, pipTrk->getZHelixK(), pipTrk->getZErrorK()); kaon
																				  ...
																				  //****************************************
	WTrackParameter wep, wem, wkp, wkm, wmp, wmm;								  //
	wep = WTrackParameter(me, pipTrk->getZHelixE(), pipTrk->getZErrorE());		  //
	wem = WTrackParameter(me, pimTrk->getZHelixE(), pimTrk->getZErrorE());		  //
	wkp = WTrackParameter(mkaon, pipTrk->getZHelixK(), pipTrk->getZErrorK());	  //
	wkm = WTrackParameter(mkaon, pimTrk->getZHelixK(), pimTrk->getZErrorK());	  //
	wmp = WTrackParameter(mmiu, pipTrk->getZHelixMu(), pipTrk->getZErrorMu());	  //
	wmm = WTrackParameter(mmiu, pimTrk->getZHelixMu(), pimTrk->getZErrorMu());	  //
#pragma endregion
#pragma region fourc_初始参数定义
	...
	double chisq_4c_e = 9999;
	double chisq_4c_k = 9999;
	double chisq_4c_m = 9999;
#pragma endregion
	...
	if (job_do_4c_3 && nGam > 2 && g2 != 0)
	{
		for (int i1 = 0; i1 < nGam; i1++)
		{
			...
			for (int i2 = i1; i2 < nGam; i2++)
			{
				...
				RecEmcShower *g2Trk = (*(evtRecTrkCol->begin() + iGam[i2]))->emcShower();
				kmfit->init();
				kmfit->AddTrack(0, wmp);
				kmfit->AddTrack(1, wmm);
				kmfit->AddTrack(2, 0.0, g1Trk);
				kmfit->AddTrack(3, 0.0, g2Trk);
				kmfit->AddFourMomentum(0, ecms);
				bool oksq = kmfit->Fit();
...
#pragma region fourc_信息输出
	if (g2 != 0)
	{
		...
		fit4c_chisq_e = chisq_4c_e;
		fit4c_chisq_k = chisq_4c_k;
		fit4c_chisq_m = chisq_4c_m;

		fit4c_gamma1_heli = my_helicityangle(fit4c_gamma1, fit4c_piz);
		fit4c_gamma2_heli = my_helicityangle(fit4c_gamma2, fit4c_piz);
		fit4c_a_pippim = my_angle(fit4c_pip, fit4c_pim);
		...
		if (job_selection)
		{
			if (fabs(fit4c_piz_m - 0.13498) < 0.045 && fit4c_pip_ep < 0.9 && fit4c_pim_ep < 0.9 && fit4c_chisq < 50)
			{
				if (fabs(fit4c_a_pippim) < 0.95 && fit4c_chisq_1g > 10 && fit4c_chisq_4g > 10)
				{
					m_tuple_selection->write();
					Ncut4++;
